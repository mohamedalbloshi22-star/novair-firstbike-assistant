-- NSR-1 V2: atomic quota reservation, safe release, rollover, and alerts
-- Run AFTER nsr1-packages-v1.sql

create table if not exists public.nsr_usage_alerts (
  id uuid primary key default gen_random_uuid(),
  client_id uuid not null references public.clients(id) on delete cascade,
  cycle_start date not null,
  threshold integer not null check (threshold in (70,85,95,100)),
  created_at timestamptz not null default now(),
  unique(client_id,cycle_start,threshold)
);

create or replace function public.nsr_roll_cycle_if_needed(p_client_id uuid)
returns void
language plpgsql
security definer
...
begin
  ...
  if current_date < v_sub.cycle_end then return; end if;

  v_start := v_sub.cycle_end;
  v_end := (v_sub.cycle_end + interval '1 month')::date;

  while current_date >= v_end loop
    v_start := v_end;
    v_end := (v_end + interval '1 month')::date;
  end loop;

  update public.nsr_client_subscriptions
...
begin
  ...
  if v_plan.hard_cap and v_before >= v_limit then
    return query select false,v_before,v_limit,0,100::numeric,'CAP_REACHED'::text,v_sub.cycle_start,v_sub.cycle_end,100;
    return;
  end if;
  ...
  if v_before::numeric/v_limit < .70 and v_used::numeric/v_limit >= .70 then v_threshold := 70;
  elsif v_before::numeric/v_limit < .85 and v_used::numeric/v_limit >= .85 then v_threshold := 85;
  elsif v_before::numeric/v_limit < .95 and v_used::numeric/v_limit >= .95 then v_threshold := 95;
...
create or replace function public.nsr_release_ai_response(p_client_id uuid)
returns void
language plpgsql
security definer
set search_path=public
...
-- Refresh current-usage function so cycle rollover occurs before reporting.
