...
const OUTPUT_PRICE_PER_MILLION =
  15;
  ...
  try {
    ...
    const [
      ...
      await Promise.all([
        ...
        supabaseRequest(
          `unanswered_dashboard_stats` +
          `?client_id=eq.${encodeURIComponent(clientId)}` +
          `&select=*`
        ),

        supabaseRequest(
          `ai_resolution_dashboard_stats` +
          `?client_id=eq.${encodeURIComponent(clientId)}` +
          `&select=*`
        ),

        supabaseRequest(
          `language_dashboard_stats` +
          `?client_id=eq.${encodeURIComponent(clientId)}` +
          `&select=*`
        ),

        supabaseRequest(
          `ai_usage_dashboard_stats` +
          `?client_id=eq.${encodeURIComponent(clientId)}` +
          `&select=*`
        )
      ]);
      ...
      .json({
        ...
        operations: {
          ...
          total_contact_requests:
            Number(
              summary.total_contact_requests ||
              0
            ),
          ...
          human_handoff_requests:
            Number(
              summary.human_handoff_requests ||
              0
            ),

          total_unanswered_questions:
            Number(
              unanswered.total_unanswered_questions ||
              0
            ),

          unresolved_unanswered_questions:
            Number(
              unanswered.unresolved_unanswered_questions ||
              0
            ),

          ai_resolution_rate_percent:
            Number(
              aiResolution.ai_resolution_rate_percent ||
              0
            ),

          arabic_conversations:
            Number(
              languageStats.arabic_conversations ||
              0
            ),

          english_conversations:
            Number(
              languageStats.english_conversations ||
              0
            )
        },
        ...
        ai_usage: {
          ...
          input_cost_usd:
            Number(
              inputCostUsd.toFixed(6)
            ),

          output_cost_usd:
            Number(
              outputCostUsd.toFixed(6)
            ),
