...
async function notifyUsageThreshold(clientId, usage){
  ...
  if(![50,75,90,100].includes(threshold)) return {skipped:true,reason:'not-notifiable-threshold'};
  ...
  const subject = `NOVAIRE | تنبيه استخدام ${threshold}% — ${name}`;
  const html = `<div dir="rtl" style="font-family:Arial,Tahoma,sans-serif;line-height:1.8;color:#111827;max-width:680px;margin:auto"><h2>NOVAIRE Smart Response</h2><p>مرحبًا،</p><p>وصل استخدام المساعد الخاص بـ <b>${esc(name)}</b> إلى <b>${threshold}%</b> من الحد الشهري.</p><p>المستخدم: <b>${esc(used)}</b><br>الحد الشهري: <b>${esc(limit)}</b><br>المتبقي: <b>${esc(remaining)}</b></p>${threshold===100?'<p><b>تم الوصول إلى الحد الشهري، وقد تتوقف ردود الذكاء الاصطناعي حتى التجديد أو ترقية الباقة.</b></p>':''}<p style="font-size:12px;color:#6b7280">إشعار آلي من NOVAIRE.</p></div>`;
  ...
  try{
    ...
    try{ await logNotification({clientId,cycleStart:usage.cycle_start,type,recipient,subject,status:'failed',metadata:{threshold,error:String(error.message || error).slice(0,500)}}); }catch{}
    throw error;
  }
}
