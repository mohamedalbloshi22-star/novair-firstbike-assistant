...
function rateKey(clientId,req){
  const raw=`contact:${String(clientId||'').trim()}:${clientIp(req)}`;
  return crypto.createHash('sha256').update(raw).digest('hex');
}

async function checkContactRateLimit(clientId,req){
  ...
  if(!r.ok)throw new Error(`Supabase contact rate limit RPC ${r.status}: ${text}`);
