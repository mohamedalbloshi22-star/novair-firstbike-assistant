...
const {safeErrorLog}=require('./nsr-safe-log');
...
async function getClientBySlug(slug){
  const r=await fetch(`${SUPABASE_URL}/rest/v1/clients?slug=eq.${encodeURIComponent(slug)}&select=id,name,slug,config&limit=1`,{
...
async function reserveAiResponse(clientId){
  ...
  if(usage?.newly_crossed_threshold){
    try{await notifyUsageThreshold(clientId,usage);}catch(error){safeErrorLog('USAGE_NOTIFICATION_ERROR',error,{client_id:clientId});}
  }
  return usage;
}
