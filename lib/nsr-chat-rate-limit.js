...
function rateKey(clientId,req){
  const raw=`${String(clientId||'').trim()}:${clientIp(req)}`;
...
async function rpc(name,body){
  ...
  if(!r.ok)throw new Error(`Supabase rate limit RPC ${r.status}: ${text}`);
  const data=text?JSON.parse(text):null;
  return Array.isArray(data)?data[0]||null:data;
}

async function checkChatRateLimit(clientId,req){
  const state=await rpc('nsr_check_chat_rate_limit',{
